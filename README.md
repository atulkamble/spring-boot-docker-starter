...
```sh
sudo docker build -t atuljkamble/java-app .
```

The build uses the provided `Dockerfile` to compile the app and bundle it inside the final image tagged `atuljkamble/java-app`.

## Run the container

```sh
docker run -d -p 8080:8080 atuljkamble/java-app
```
